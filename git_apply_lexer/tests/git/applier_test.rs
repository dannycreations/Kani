use hit::applier;
use hit::error::Error;
use hit::fs::{FileSystem, MockFileSystem};
use hit::parser::{Hunk, Line, Patch};
...
"#;
  ...
  assert!(!fs.exists(&PathBuf::from("old_name.txt")));
  assert!(fs.exists(&PathBuf::from("new_name.txt")));
  assert_eq!(
...
"#;
  ...
  applier::patch(&mut fs, diff, false).unwrap();
  assert!(!fs.exists(&PathBuf::from("old_metadata.txt")));
  assert!(fs.exists(&PathBuf::from("new_metadata.txt")));
  assert_eq!(
...
"#;
  ...
  applier::patch(&mut fs, diff, false).unwrap();
  assert!(fs.exists(&PathBuf::from("new_file.txt")));
  assert_eq!(
...
"#;
  ...
  applier::patch(&mut fs, diff, false).unwrap();
  assert!(!fs.exists(&PathBuf::from("file_to_delete.txt")));
}
...
"#;
  ...
  applier::patch(&mut fs, diff, false).unwrap();
  assert!(fs.exists(&PathBuf::from("old_file.txt")));
  assert!(fs.exists(&PathBuf::from("new_file.txt")));
  assert_eq!(
    fs.read_to_string(&PathBuf::from("new_file.txt")).unwrap(),
    "content"
...
"#;
  ...
  applier::patch(&mut fs, diff, false).unwrap();
  assert!(fs.exists(&PathBuf::from("file.txt")));
  assert_eq!(
    fs.read_to_string(&PathBuf::from("file.txt")).unwrap(),
    "hello\n"
  );
...
"#;
  ...
  applier::patch(&mut fs, diff, false).unwrap();
  assert!(fs.exists(&PathBuf::from("new/dir/file.txt")));
  assert_eq!(
...
"#;
  ...
  applier::patch(&mut fs, diff, false).unwrap();
  assert!(fs.exists(&PathBuf::from("empty.txt")));
  assert_eq!(
