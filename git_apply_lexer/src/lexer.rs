...
impl<'a> Lexer<'a> {
  ...
  fn strip_git_prefix(s: &'a str) -> Result<&'a str, Error> {
    if s == "/dev/null" {
      Ok(s)
    } else if let Some(stripped) = s.strip_prefix("a/") {
      Ok(stripped)
    } else if let Some(stripped) = s.strip_prefix("b/") {
      Ok(stripped)
    } else {
      Err(Error::Parse(format!("Malformed file path: `{}`", s)))
    }
  }
    ...
    match parts.next() {
      Some(hashes) => {
        let (old_hash, new_hash) = hashes
          .split_once("..")
          .ok_or_else(|| Error::Parse("Invalid index hash range".into()))?;
      ...
      _ => Err(Error::Parse("Invalid index line".into())),
    }
  }
  ...
  fn parse_hunk_header(header: &'a str) -> Result<Token<'a>, Error> {
    let content = header
      .split(" @@")
      .next()
      .ok_or_else(|| Error::Parse("Malformed hunk header".into()))?;
    ...
    let old_range_str = parts
      ...
      .ok_or_else(|| Error::Parse("Missing old range in hunk header".into()))?;

    let new_range_str = parts
      .next()
      .and_then(|s| s.strip_prefix('+'))
      .ok_or_else(|| Error::Parse("Missing new range in hunk header".into()))?;
  ...
  fn parse_range(range_str: &str) -> Result<(u32, u32), Error> {
    ...
    let line = parts
      .next()
      .ok_or_else(|| {
        Error::Parse(format!("Missing line number in range: `{}`", range_str))
      })?
      .parse()
      .map_err(|e| {
        Error::Parse(format!(
          "Invalid hunk range line: `{}` - {}",
          range_str, e
        ))
      })?;

    let span = parts.next().map_or(Ok(1), |s| s.parse()).map_err(|e| {
      Error::Parse(format!("Invalid hunk range span: `{}` - {}", range_str, e))
    })?;
  ...
  fn parse_percentage(
    ...
    s.strip_suffix('%')
      .ok_or_else(|| {
        Error::Parse(format!("{}: Missing percentage sign", error_msg))
      })?
      .parse()
      .map_err(|e| Error::Parse(format!("{}: {}", error_msg, e)))
  }

  fn parse_octal_mode(s: &str) -> Result<u32, Error> {
    u32::from_str_radix(s, 8)
      .map_err(|e| Error::Parse(format!("Invalid file mode: {}", e)))
  }
}
...
impl<'a> Iterator for Lexer<'a> {
  ...
  fn next(&mut self) -> Option<Self::Item> {
    ...
    line.map(|line_content| {
      ...
      } else if let Some(rest) = line_content.strip_prefix("new file mode ") {
        let mode = Self::parse_octal_mode(rest)?;
        Ok(Token::NewFileMode(mode))
      } else if let Some(rest) = line_content.strip_prefix("new mode ") {
        let mode = Self::parse_octal_mode(rest)?;
        Ok(Token::NewFileMode(mode))
      ...
      } else if let Some(rest) = line_content.strip_prefix("deleted file mode ")
      {
        ...
        let (old_file, rest) = rest
          .split_once(" and ")
          .ok_or_else(|| Error::Parse("Invalid binary files line".into()))?;
        let new_file = rest
          .strip_suffix(" differ")
          .ok_or_else(|| Error::Parse("Invalid binary files line".into()))?;
        ...
        line_content.strip_prefix("dissimilarity index ")
      {
        let percent = Self::parse_percentage(rest, "Invalid dissimilarity")?;
        Ok(Token::Dissimilarity(percent))
      } else {
        Err(Error::Parse(format!("Unexpected line: `{}`", line_content)))
      }
    })
  }
}
