...
impl AgileEncryptionInfo {
  fn from_xml(xml_str: &str) -> Result<Self> {
    ...
    if alg_id.as_deref() != Some("SHA512") {
      bail!("Unsupported hash algorithm (only SHA512 supported)");
    }

    Ok(Self {
      key_data_salt: key_data_salt.context("Missing keyData salt")?,
      enc_key_salt: enc_key_salt.context("Missing encryptedKey salt")?,
      spin_count: spin_count.context("Missing spinCount")?,
  ...
  W: Write,
{
  ...
  let enc_info_stream = cfb.open_stream("/EncryptionInfo")?;
  let mut enc_info_data = Vec::new();
  enc_info_stream
    .take(MAX_ENCRYPTION_INFO_SIZE as u64)
    .read_to_end(&mut enc_info_data)?;

  if enc_info_data.len() < 8 {
    bail!("EncryptionInfo stream too short");
  }
  ...
  loop {
    ...
    let to_write = if bytes_decrypted + pos > total_size {
      &chunk[..(total_size.saturating_sub(bytes_decrypted))]
    } else {
      chunk
    };

    writer.write_all(to_write)?;
    bytes_decrypted = bytes_decrypted.saturating_add(to_write.len());
  }
